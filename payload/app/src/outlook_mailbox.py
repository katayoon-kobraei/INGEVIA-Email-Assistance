...
def get_target_folder(namespace: Any, folder_id: int, target_mailbox: str | None = None) -> Any:
    ...
    accessible. Temporary Account/Store/Recipient COM wrappers are released
    before returning the selected folder, reducing MAPI pressure during repeated
    scheduled runs.
    """
    ...
    recipient = accounts = account = stores = store = None
    resolved_name = ""
    try:
        try:
        ...
        # Releasing these wrappers never closes Outlook or the returned folder;
        # it only drops temporary COM references used to locate the mailbox.
        store = None
        account = None
        stores = None
        accounts = None
