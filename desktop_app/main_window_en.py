...
class EmailsPage(QWidget):
    def __init__(self) -> None:
        ...
        self.search.setPlaceholderText("Search by subject, contact, or project...")
        self.project_filter = QComboBox()
        self.project_filter.addItem("All projects")
        self.direction_filter = QComboBox()
        self.direction_filter.addItems(["All directions", "Incoming", "Outgoing"])
        self.status_filter = QComboBox()
        self.status_filter.addItems(["All statuses", "Processed", "Review"])
...
class AttachmentsPage(QWidget):
    ...
    def _render(self) -> None:
        ...
        for record in self.rows:
            haystack = " ".join(str(record.get(key, "")) for key in ("Project", "Subject", "File", "Reason")).lower()
            if query and query not in haystack:
                continue
...
class SettingsPage(QWidget):
    def __init__(self) -> None:
        ...
        self.archive_value = add_setting_row(layout, "Archive folder", str(data_service.ARCHIVE_ROOT))
...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self) -> QFrame:
        ...
        layout.addStretch()
        status = QLabel("●  Local application")
    ...
    def refresh_outlook_flags(self) -> None:
        if self.outlook_worker and self.outlook_worker.isRunning():
            return
        self.outlook_pill.setText("Outlook: checking...")
        self.outlook_pill.setObjectName("WarningPill")
    ...
    def _outlook_loaded(self, summary: data_service.OutlookFlagSummary) -> None:
        ...
        if summary.error:
            self.outlook_pill.setText("Outlook: unavailable")
            self.outlook_pill.setObjectName("ErrorPill")
        else:
            self.outlook_pill.setText(f"Outlook: connected · {summary.total} flagged")
